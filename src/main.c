...
void initPhysicsWorld(void)
{
    ...
    block0BodyDef.position = (b2Vec2){ 205.f / pixelsPerMeter, 120.f / pixelsPerMeter };
    ...
    //     (b2Vec2) { 20.f / pixelsPerMeter, -10.f / pixelsPerMeter }
    // };
    // b2Hull block2Hull = b2ComputeHull(block2Points, 4);
    // b2Polygon block2Shape = b2MakePolygon(&block2Hull, 0.f);
    ...
    //     (b2Vec2) { 20.f / pixelsPerMeter, -10.f / pixelsPerMeter }
    // };
    // b2Hull block4Hull = b2ComputeHull(block4Points, 4);
    // b2Polygon block4Shape = b2MakePolygon(&block4Hull, 0.f);
